use std::{collections::HashMap, fs::read, io::Read, net::{SocketAddr, ToSocketAddrs}, sync::Arc, time::Duration};
...
async fn create_tunnel(uuid:Uuid){
	...
	let (mut tunnel_read, mut tunnel_write) = create_new_connection(PROXY_LOCATION.to_owned()+":"+TUNNEL_PORT).await;
	let mut data = BytesMut::new();
	loop {
...
#[tokio::main]
async fn main() {
	let addr = (PROXY_LOCATION.to_owned()+":"+TUNNEL_PORT).to_socket_addrs().unwrap().collect::<Vec<SocketAddr>>()[0];
    let socket = TcpSocket::new_v4().unwrap();
    let stream = socket.connect(addr).await.unwrap();
